...
function EditPage() {
  ...
  const loadPage = async () => {
    try {
      ...
      const response = await pageApi.getPageById(id, false);
      ...
      if (response.success) {
        ...
        setFormData({
          ...
          og_image: null, // File input will be handled separately
        ...
        // Set existing OG image if available
        if (page.og_image) {
          setExistingOgImage(page.og_image);
          setOgImagePreview(page.og_image);
        }
      } else {
  ...
  const handleClearImage = () => {
    ...
    setOgImagePreview(existingOgImage); // Revert to existing image
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add old OG image path for cleanup if new image is uploaded
      if (formData.og_image && existingOgImage) {
        updateData.old_og_image = existingOgImage;
      }

      const response = await pageApi.updatePage(id, updateData);
      ...
      if (error.response?.data?.errors) {
        ...
        setErrors({ general: 'Failed to update page. Please try again.' });
      }
    } finally {
      setLoading(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled
                      className="form-input w-full bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400"
                    >
                      {languages.map(lang => (
                  ...
                  <div className="space-y-4">
                    ...
                    {formData.og_image && (
                      <div className="text-sm text-gray-600 dark:text-gray-400">
                        <p><strong>New File:</strong> {formData.og_image?.name}</p>
        ...
        <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700/60">
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 text-white hover:from-blue-600 hover:via-indigo-600 hover:to-purple-600 disabled:opacity-50"
          >
            {loading ? (
